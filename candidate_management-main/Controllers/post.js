      ...
      for (let j = i + 1; j < info.length; j++) {
        if (
          ...
          if (!arr.includes(info[i].candidate_name)) {
            duptable.push({
              candidate_name: info[i].candidate_name,
              email: info[i].email,
              contact: info[i].contact,
              pan: info[i].pan,
              role: info[i].role,
              it_experience_years:
                info[i].it_experience_years,
            });
        ...
        Candidate.addCandid.insertMany(
          ...
          (err, data) => {
            if (err) {
              ...
              else return res.status(200).json(duptable);
...
const updateCandidateStatus = async (req, res) => {
  ...
  try {
    const candidate = await Candidate.addCandid.updateOne(
      { id: candidateid },
      {
        $set: {
          candidate_status_id: updatedStatus,
        },
      },
    );

    console.log(candidate, "Hi");
