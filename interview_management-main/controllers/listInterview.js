...
module.exports.Search = async function (req, res) {
  try {
    ...
    if (req.query.id) {
      dbConstraint1.id = req.query.id;
    }

    let dbConstraint2 = {};

    if (req.query.role_name) {
      dbConstraint2["roles.role_name"] = {
        $regex: req.query.role_name,
        $options: "i",
      };
    }
    ...
    if (req.query.type) {
      dbConstraint5["interview_type.type"] = {
    ...
    let dbConstraint6 = {};
    if (req.query.status) {
      dbConstraint6["interview_status.interview_status"] =
        req.query.status;
    }
    ...
    const totalrecords = await Interview.aggregate([
...

      {
        $lookup: {
          ...
          foreignField: "interview_status_id",
