...
function SingleList({ hash, role, index, arr }) {
  ...
  return (
    ...
    <tr className={!check ? "table-inactive" : ""}>
      ...
      <td align="center">{role.user_id}</td>

      <td align="center">{role.name}</td>

      <td align="center">{role.email}</td>

      <td align="center">{role.contact}</td>

      <td align="center">{role.role_name}</td>

   

      <td align="center">{role.available_date.split('T')[0]}</td>

      <td align="center">{role.start_time} - {role.end_time}</td>

      <td align="center">{statuss}</td>



      {userData.role_id == 1003 ? <></> : <td align="center">

        {check && <Link to={`/UpdatePanel/${role.user_id}/${role.name}/${role.available_date.split("T")[0]}/${role.grade}/${role.
