 ...
 */
  ...
  useEffect(() => {
    setLoggedUser(JSON.parse(window.sessionStorage.getItem('user_data')));
  }, []);
  ...
  const getCsvData = async () => {
    ...
    try {
      ...
      const finaldata = roleListData.data.map((item) => {
        const { user_id, name, email, contact, role_name, type, available_date, start_time, end_time, availability_status } = item
        return { user_id, name, email, contact, role_name, type, available_date: available_date.split('T')[0], slot: `${start_time}-${end_time}`, availability_status }
      })
  ...
  useEffect(() => {
    let temp = []
    for (let i = 1; i <= Math.ceil(totalPages / 10); i++) { if (pagee === i) { temp.push(<button className="btn btn-primary col-sm-3" onClick={(e) => { e.preventDefault(); dispatch(panavailActions.changePage(i)); }}>{i}</button>); } else { temp.push(<button className="btn btn-outline-primary col-sm-3" onClick={(e) => { e.preventDefault(); dispatch(panavailActions.changePage(i)); }}>{i}</button>) } } dispatch(panavailActions.getButtons([...temp]));
  }, [pagee])
  ...
  function gotoMult() {
    console.log(1);
    if (loggedUser.role_id === '1001') { navigate('/phmultiple') }
